...
public class UserServiceTests
{
    ...
    public async Task Get_All_Users_Should_Return_An_Array_Of_Users()
    {
        ...
        _mockRepository.Setup(u => u.GetAllAsync()).Returns(mockUsers.BuildMock());
    ...
    public async Task Get_User_Groups_Should_Return_An_Array_Of_Groups_The_User_Is_In()
    {
        ...
        _mockGroupRepository.Setup(g => g.GetAllIncludingAsync(g => g.Users.Where(u => u.Id == "1"))).Returns(mockGroups.BuildMock());
        _mockGroupRepository.Setup(g => g.SaveChanges()).Verifiable();

        List<Group> groups = await _service.GetUserGroups("1");
    ...
    public async Task Get_User_Sets_Should_Return_A_List_Of_Sets_That_Contains_The_User()
    {
        ...
        _mockSetRepository.Setup(s => s.GetAllIncludingAsync(s => s.Users.Where(u => u.Id == "2"))).Returns(mockSets.BuildMock());

        List<Set> sets = await _service.GetUserSets("2");
